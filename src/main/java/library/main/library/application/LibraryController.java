...
import library.main.library.ViewBook;
...
public class LibraryController implements Initializable {
    ...
    public void doubleClick() throws IOException {
        booksList.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                if(event.getButton() == MouseButton.PRIMARY && event.getClickCount() == 2 &&
                        (event.getTarget() != null || ((GridPane) event.getTarget()).getChildren().size() > 0)) {
                    System.out.println(booksList.getSelectionModel().getSelectedItem());
                    try {
                        ViewBook.changeScene(booksList.getSelectionModel().getSelectedItem());
                    } catch (IOException e) {
